#!/usr/bin/env python3
"""
Re-run CALANet experiments with fixed checkpoint loading
This should get results ~5-10% closer to paper values
"""
...
# CALANet experiments to re-run (12 total)
EXPERIMENTS = [
...
def run_experiment(model, dataset, task_type, script_path):
    ...
    temp_script = os.path.join(script_dir, f'temp_{dataset}_fixed_{script_name}')
    ...
    try:
        ...
        # Use the rthar conda environment Python
        ...
        if success and has_metrics:
            ...
            if "Loaded best model" in result.stdout:
                print(f"   ✓ Best checkpoint loaded successfully")
            else:
                print(f"   ⚠ Warning: Checkpoint loading message not found in output")
...
def main():
    print("="*80)
    print("RE-RUNNING CALANet WITH FIXED CHECKPOINT LOADING")
    print("="*80)
    print("Fix: Load best checkpoint before metrics collection")
    print(f"Total experiments: {len(EXPERIMENTS)}")
    print("Expected improvement: ~5-10% closer to paper values")
    print("="*80)
    print()
    ...
    print("="*80)
    print("\nNext step: Compare new results with paper values")
    print("Run: python3 compare_results.py")
    print("="*80)
