"""
HAR Dataset Experiments using SAGoG
...
# SAGoG expects input shape (batch_size, num_variables, seq_len)
# Our data is already in (batch_size, channels, seq_len) format
num_variables = input_nc
seq_len = segment_size

# Adjust number of windows based on sequence length
num_windows = max(3, min(10, segment_size // 30))
...
os.makedirs('SAGOG/results', exist_ok=True)
with open(f'SAGOG/results/{dataset}_sagog_results.txt', 'w') as f:
