...
model = If_ConvTransformer(1, input_nc+3, 64, 5, 3,2, 128, 1, 0.2, segment_size, class_num).cuda()
...
metrics_collector = MetricsCollector(
    model_name='IF-ConvTransformer2',
    dataset=dataset,
    task_type='HAR',
    save_dir='results'
)
...
with metrics_collector.track_training():
    for epoch in range(epoches):
        ...
        if max_f1 < weighted_avg_f1:
            os.makedirs('IF-ConvTransformer2/save', exist_ok=True)
            torch.save(model.state_dict(), 'IF-ConvTransformer2/save/'+dataset + memo + '.pt')
