...
from interpretability_metrics import calculate_aopcr, calculate_ndcg_at_n
from util import custom_tqdm, cross_entropy_criterion
...
class MILLETModel:
    ...
    def fit(
        ...
        for _ in custom_tqdm(range(n_epochs), desc="Training model"):
            ...
            for (bags, targets) in train_dataset:
                #bags = batch["bags"]
                #targets = batch["targets"].to(self.device)
                bags = bags.type(torch.cuda.FloatTensor)
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for (bags, targets) in dataloader:
                #bags = batch["bags"]
        ...
        results = classification_report(all_targets.long(), all_pred_clzs, digits=4, output_dict=True)
    ...
    def evaluate_interpretability(
        ...
        #dataloader = dataset.create_dataloader(batch_size=16)
        with torch.no_grad():
            for (bags, targets) in custom_tqdm(dataloader, leave=False):
                #bags = batch["bags"]
