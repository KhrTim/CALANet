...
model = RepHAR(input_nc, class_num).cuda()
...
with metrics_collector.track_training():
    for epoch in range(epoches):
        ...
        # evaluating
        eval_loss, y_pred = infer(eval_queue, copy.deepcopy(model), criterion)
        ...
        if max_f1 < weighted_avg_f1:
            #torch.save(model.state_dict(), 'SlowFastNet/save/'+dataset+'.pt')
