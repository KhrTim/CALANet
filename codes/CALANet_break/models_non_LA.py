...
class TAggBlock(nn.Module):
    def __init__(self, i_nc, o_nc, L, T, pool):
        ...
        self.gconv = nn.Conv1d(i_nc, o_nc, kernel_size=5, padding='same', bias=False)
        self.bn = nn.BatchNorm1d(o_nc)

        # temporal glance convolution
        #self.tgconv = nn.Conv1d(o_nc, o_nc, kernel_size=T)
...
class HTAggNet(nn.Module):
    def __init__(self, nc_input, n_classes, segment_size, L):
        ...
        self.stem = nn.Conv1d(nc_input, first_nc, 5, padding='same')    
        ...
        for i in range(self.L):
            if i in pool_idx:
                ...
                pool = False
            if i == 0:
                self.layers.append(TAggBlock(first_nc, nc_o, self.L, T, pool))
            else:
                self.layers.append(TAggBlock(nc_i, nc_o, self.L, T, pool))
            nc_i = nc_o
        
        self.fc = nn.Linear(nc_o, n_classes)
