...
class Masking(object):
    def __init__(self, optimizer, death_rate=0.3, growth_death_ratio=1.0, death_rate_decay=None, death_mode='magnitude', growth_mode='momentum', redistribution_mode='momentum', threshold=0.001, train_loader=None, T_max=0.):
        ...
        self.prune_every_k_steps = 5 * len(self.loader)
        ...
        if mode == 'GMP':
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    nn = int(39 / 3)
                    k_s = [nn * (i + 1) for i in range(3)]
                    mask_chunk = torch.chunk(mask, 3, dim=0)
                    mask_list = []
                    for i, w_c in enumerate(mask_chunk):
                        kernel_size = k_s[i]
                        search_num = (mask_chunk[i] == 0).sum().item()*kernel_size/39
    ...
    def step(self):
        ...
        if self.prune_every_k_steps is not None:
            ...
            if self.steps % self.prune_every_k_steps == 0:
                ...
                print('curr_density: {0:.4f}, final_density:{1:.4f}'.format(self.curr_density, 0.1))
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                if self.death_mode == 'magnitude':
                    new_mask, remove_list = self.magnitude_death_channel(mask, weight, name, c_size=3)
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                if self.growth_mode == 'random':
                    new_mask = self.random_growth_channel(name, new_mask, weight, c_size=3, num_rm_list=self.num_rm_list[name])
                    ...
                    new_mask = self.gradient_growth_channel(name, new_mask, weight, c_size=3, num_rm_list=self.num_rm_list[name])
